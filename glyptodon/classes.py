...
@patch
def __init__(self: Line, x0, y0, x1, y1, index=-1, text="none"):
    self.x0 = int(x0)
    self.y0 = int(y0)
    self.x1 = int(x1)
    self.y1 = int(y1)
...
def linesToCSV(cls, targetDirectory, lines, fileName):
    ...
    writer = csv.writer(f)
    for line in lines:
        writer.writerow([line.x0, line.y0, line.x1, line.y1, line.index, line.text])
...
def csvToLines(cls, targetFile):
    ...
    for row in csvReader:
        if len(row) == 6:
            line = Line(
                x0=row[0], y0=row[1], x1=row[2], y1=row[3], index=row[4], text=row[5]
            )
        else:
